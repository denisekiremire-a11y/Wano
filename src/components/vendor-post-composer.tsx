...
import { createVendorPostAction } from "@/lib/actions/vendor-post-actions";
...
export function VendorPostComposer({ listings }: { listings: { id: string; title: string }[] }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await createVendorPostAction({}, formData);
      setState(result);
      if (!result.error) {
        setContent("");
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl border border-forest-900/10 bg-white p-4">
      <textarea
        ...
        placeholder="Share an update — a new dish, a special, an event you're hosting…"
      ...
      {listings.length > 0 && (
        <label className="mt-2 flex items-center gap-2 text-xs text-forest-800/60">
          About
          <select
            value={listingId}
      ...
      <div className="mt-1 text-right text-[11px] text-forest-800/40">
      ...
      {images.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-2">
          {images.map((img, i) => (
            <div key={img.previewUrl} className="relative h-16 w-16 overflow-hidden rounded-lg border border-forest-900/15">
      ...
      {posted && <p className="mt-1 text-xs text-forest-700">Posted — live on your page and the Social feed now.</p>}
    </form>
  );
}
